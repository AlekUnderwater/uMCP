...
namespace uMCPTest
{
    public partial class MainForm : Form
    {
        ...
        {
            sendBtn.Enabled = (!string.IsNullOrEmpty(textToSendTxb.Text)) && (port != null) && (port.IsOpen) && (port.State == uMCPState.RUNNING);            
        }
        ...
        private void connectBtn_Click(object sender, EventArgs e)
        {
            if ((port == null) || (!port.IsOpen))
            {
                try
                {
                    ...
                    port = new uMCPPort(portNameCbx.SelectedItem.ToString(),
                        UCNLDrivers.BaudRate.baudRate9600, 0, isSelectedByDefaultChb.Checked, 8100, 2000, 64);
                    ...
                    { 
                        ...
                        InvokeSetEnabledState(mainToolStrip, startBtn, (port.State == uMCPState.RUNNING) || (port.State == uMCPState.HALTED));
                        InvokeCheckSendEnabled();

                        if (port.State == uMCPState.RUNNING)
                            InvokeSetText(mainToolStrip, startBtn, "STOP");
                        else if (port.State == uMCPState.HALTED)
        ...
        {
            if (port.State == uMCPState.HALTED)
                port.Start();
            else
                port.Stop();
        }
